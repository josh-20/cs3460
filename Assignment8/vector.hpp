...
namespace usu
{
    ...
    class vector
    {
      ...
      private:
        ...
        pointer m_data;
        resize_type m_resize = [](size_type currentCapacity) -> size_type
        {
    ...
    void vector<T>::remove(size_type index)
    {
        ...
        else
        {
            if (index != m_size - 1)
            {
                for (auto i = index; i < this->m_size - 2; i++)
                {
                    m_data[i] = m_data[i + 1];
                }
                m_data[m_size - 2] = m_data[m_size - 1];
            }
            this->m_size--;
        }
    }
    ...
    {
        return iterator(this->m_size, this->m_data);
    }
